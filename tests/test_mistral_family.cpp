...
//
// Purpose:
//   Validates the end-to-end integration of the Mistral model family through the
//   HF family registry. This exercises Registry 1 (HfModelFamily) and Registry 2
//   (CheckpointMapper) for the Mistral architecture, ensuring that a synthetic HF
//   model directory with model_type "mistral" is correctly detected, loaded, and
//   mapped to a DecoderModel.
//
// What is verified:
//   - HF family detection: model_type "mistral" + "MistralForCausalLM" triggers
//     the Mistral family registration.
//   - Multi-layer checkpoint loading: a 2-layer safetensors checkpoint is read
//     and mapped through MistralCheckpointMapper.
//   - GQA layout: num_kv_heads (1) < num_attention_heads (2), verifying grouped
//     query attention geometry is preserved.
//   - No per-head QK norms: Mistral does not use q_norm/k_norm, so these vectors
...
//   - Mistral family registration (auto-discovered via CMake GLOB)
//
// Approach:
//   Creates a synthetic 2-layer Mistral checkpoint in a temporary directory with
...
//   Structurally similar to test_llama_family since Mistral shares the same
//   Pre-RMSNorm+GQA+RoPE+SwiGLU decoder pattern.
// =============================================================================
...
// ---------------------------------------------------------------------------
// write_hf_mistral_root
//
// Intention: Create a minimal but structurally valid HF Mistral model directory.
// Setup:
//   - Writes config.json with model_type="mistral", architectures=["MistralForCausalLM"],
...
int main()
{
    ...
    try
    {
        ...
        //
        // Intention: Verify that the family registry recognizes the Mistral
        ...
        //
        // Intention: Confirm that the Mistral checkpoint mapper successfully
        ...
        // -----------------------------------------------------------------
        // Assertion: Family is detected as "mistral"
        //
        // Intention: Verify that the HF family registry matched model_type
        //   "mistral" and set the architecture.family field accordingly.
        // Mechanism: Checks that the family string starts with "mistral".
        // -----------------------------------------------------------------
        ...
        // -----------------------------------------------------------------
        // Assertion: q_norm and k_norm are empty (Mistral has no QK norms)
        //
        // Intention: Mistral does not use per-head RMSNorm on Q/K projections
