"""Contract test plugin for speech-to-speech models (PersonaPlex)."""
    ...
    def verify(self, trt_output, ref_output, case, threshold):
        ...
        if trt_rms is not None:
            ...
            metrics["rms"] = MetricResult(
                value=float(trt_rms), threshold=min_rms, operator=">=", passed=rms_ok)

        if trt_duration is not None:
            dur_ok = float(trt_duration) >= 0.1
            metrics["duration_s"] = MetricResult(
                value=float(trt_duration), threshold=0.1, operator=">=", passed=dur_ok)
        ...
        if trt_tokens is not None and ref_tokens is not None:
            ...
            if min_len > 0:
                match = sum(1 for a, b in zip(trt_tokens[:min_len], ref_tokens[:min_len]) if a == b) / min_len
                token_threshold = threshold.metrics.get("contract_token_match", 0.5)
                metrics["token_match"] = MetricResult(
                    value=match, threshold=token_threshold, operator=">=", passed=match >= token_threshold)

        all_passed = all(m.passed for m in metrics.values())
        if all_passed:
            return make_pass("full_generation", metrics, "audio health + token match")
        return make_fail("full_generation", metrics, "audio health + token match",
                        "Speech response health check failed")

plugin = SpeechToSpeechPlugin()
