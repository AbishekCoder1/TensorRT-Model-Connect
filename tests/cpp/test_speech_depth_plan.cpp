...
void test_projection_view_derives_dimensions_and_flag()
{
    ...
    check(view.temporal_hidden_dim == 3, "speech depth projection view tracks temporal hidden size");
    check(view.proj_size_per_cb == 6, "speech depth projection view computes per-codebook projection span");
}
