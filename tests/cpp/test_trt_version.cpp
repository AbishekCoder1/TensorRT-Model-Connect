...
static void test_parse_version() {
    auto v = trtf::parse_trt_version("10.15.0.6");
    ...
    check_eq(trtf::trt_abi_string(*v), "10.15", "format ABI string");
    check_eq(trtf::trt_abi_suffix(*v), "10_15", "format ABI suffix");
    check_eq(trtf::trt_backend_name_for_abi(*v), "trt_10_15", "format backend name");

    check(!trtf::parse_trt_version("unknown").has_value(), "unknown version does not parse");
}

static void test_parse_abi_tag() {
    auto dotted = trtf::parse_trt_abi_tag("10.16");
    check(dotted.has_value(), "parse dotted ABI tag");
    check(dotted->major == 10 && dotted->minor == 16, "dotted ABI values");

    auto backend = trtf::parse_trt_abi_tag("trt_11_0");
...
static void test_matching() {
    auto a = *trtf::parse_trt_version("10.15.0");
    auto b = *trtf::parse_trt_version("10.15.2");
    auto c = *trtf::parse_trt_version("10.16.0");
    check(trtf::trt_abi_matches(a, b), "patch difference does not change ABI");
    check(!trtf::trt_abi_matches(a, c), "minor difference changes ABI");
}

static void test_candidates() {
    auto required = trtf::parse_trt_version("10.15.0");
    auto installed = trtf::parse_trt_version("10.16.0");

    std::vector<std::string> candidates = trtf::trt_backend_candidates("trt", required, installed);
    ...
    candidates = trtf::trt_backend_candidates("trt", std::nullopt, installed);
    ...
    candidates = trtf::trt_backend_candidates("trt_11_0", required, installed);
    ...
    candidates = trtf::trt_backend_candidates("trt_rtx", required, installed);
