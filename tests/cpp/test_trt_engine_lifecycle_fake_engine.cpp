...
std::vector<std::string> make_required_tensor_names(int32_t num_layers,
    ...
    for (int32_t i = 0; i < num_layers; ++i) {
        names.emplace_back(trtmc::layer_tensor_name("cache_k", i));
        names.emplace_back(trtmc::layer_tensor_name("cache_v", i));
        names.emplace_back(trtmc::layer_tensor_name("present_k", i));
        names.emplace_back(trtmc::layer_tensor_name("present_v", i));
    }
    return names;
}
...
trtmc::DecoderStepEngine make_decoder_step_engine(FakeModule& module, int32_t num_layers,
    ...
    for (int32_t i = 0; i < num_layers; ++i) {
        engine.cache_k_input_names.push_back(trtmc::layer_tensor_name("cache_k", i));
        engine.cache_v_input_names.push_back(trtmc::layer_tensor_name("cache_v", i));
        engine.present_k_output_names.push_back(trtmc::layer_tensor_name("present_k", i));
        engine.present_v_output_names.push_back(trtmc::layer_tensor_name("present_v", i));
    }
    return engine;
}
...
void test_has_io_tensor_true_false() {
    ...
    check(trtmc::has_io_tensor(module, "token_id"), "has_io_tensor returns true for existing name");
    check(!trtmc::has_io_tensor(module, "logits"), "has_io_tensor returns false for missing name");
    check(!trtmc::has_io_tensor(module, ""), "has_io_tensor returns false for empty name");
}
...
void test_has_all_required_tensors_false_when_missing_base_tensors() {
    ...
    trtmc::DecoderStepEngine engine = make_decoder_step_engine(module, /*num_layers=*/2, true);
    check(!trtmc::has_all_required_tensors(engine),
...
void test_has_all_required_tensors_requires_position_input_branch() {
    ...
    trtmc::DecoderStepEngine no_position_required =
        make_decoder_step_engine(no_position_module, /*num_layers=*/1, false);
    check(trtmc::has_all_required_tensors(no_position_required),
    ...
    FakeModule missing_position_module(make_required_tensor_names(
        /*num_layers=*/1, /*include_position_input=*/false));
    trtmc::DecoderStepEngine position_required_missing =
        make_decoder_step_engine(missing_position_module, /*num_layers=*/1, true);
    check(!trtmc::has_all_required_tensors(position_required_missing),
...
void test_has_all_required_tensors_missing_per_layer_tensor_fails() {
    ...
    remove_name(names, trtmc::layer_tensor_name("present_v", 1));
    FakeModule module(std::move(names));
    trtmc::DecoderStepEngine engine = make_decoder_step_engine(module, /*num_layers=*/3, true);
    check(!trtmc::has_all_required_tensors(engine),
