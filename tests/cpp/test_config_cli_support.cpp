...
void register_demo_schema()
{
    ...
    reg.register_schema(Schema{"triattention", {
        int_field("kv_budget", 6144,
                  {Layer::SessionRequest, Layer::PlatformProfile, Layer::BundleDefault}),
        bool_field("protect_prefill", true,
                   {Layer::SessionRequest, Layer::BundleDefault}),
        string_field("dump_scores_path", "",
...
{
    expect_throws([] { trtf::config::parse_set_token("ns.field42"); },
                  "missing '='", "parse_set_token_missing_equals");
}

void test_parse_set_token_missing_dot()
{
    expect_throws([] { trtf::config::parse_set_token("nsfield=42"); },
                  "missing '.'", "parse_set_token_missing_dot");
}

void test_parse_set_token_empty_parts()
{
    expect_throws([] { trtf::config::parse_set_token(".field=42"); },
                  "empty", "parse_set_token_empty_ns");
    expect_throws([] { trtf::config::parse_set_token("ns.=42"); },
                  "empty", "parse_set_token_empty_field");
}
...
{
    expect_throws([] { trtf::config::coerce_scalar("3.14", "int32", "x.y"); },
                  "expected integer", "coerce_int_rejects_float_text");
}

void test_coerce_float()
{
    auto v = trtf::config::coerce_scalar("3.14", "float", "x.y");
    check(std::any_cast<float>(v) > 3.13F && std::any_cast<float>(v) < 3.15F,
          "coerce_float");
}
...
{
    expect_throws([] { trtf::config::coerce_scalar("maybe", "bool", "x.y"); },
                  "expected bool", "coerce_bool_rejects_unknown");
}
...
{
    expect_throws([] { trtf::config::parse_layered_json("{ \"ns\" { } }"); },
                  "expected ':'", "parse_missing_colon");
    expect_throws([] { trtf::config::parse_layered_json("{ \"ns\": [1,2] }"); },
                  "expected '{'", "parse_rejects_array");
...
void test_build_cli_contribution_coerces_set_tokens()
{
    ...
    check(std::any_cast<std::string>(contrib.values.at("triattention").at("dump_scores_path")) == "/tmp/x.pt",
...
void test_set_overrides_config_within_session_layer()
{
    ...
    file_values["triattention"]["kv_budget"] = std::any{std::int64_t{4096}};
    auto contrib = trtf::config::build_cli_contribution(
        file_values, {"triattention.kv_budget=8192"});
    check(std::any_cast<std::int32_t>(contrib.values.at("triattention").at("kv_budget")) == 8192,
...
void test_unknown_field_raises()
{
    register_demo_schema();
    expect_throws([] {
        trtf::config::build_cli_contribution({}, {"triattention.nope=1"});
...
{
    register_demo_schema();
    const std::string path = make_temp_json(R"({"triattention":{"kv_budget":4096,"protect_prefill":false}})");
    auto bundle = trtf::config::resolve_cli_config(
        path, {"triattention.kv_budget=8192"});
...
void test_resolve_session_beats_platform()
{
    ...
    auto bundle = trtf::config::resolve_cli_config(
        "", {"triattention.kv_budget=8192"}, {platform});
    check(bundle.get<std::int32_t>("triattention", "kv_budget") == 8192,
...
void test_write_effective_config_next_to_places_file(std::string tmp_dir)
{
    ...
    std::string written = trtf::config::write_effective_config_next_to(
