...
void test_conditioning_inputs_build_mask_and_null_ids()
{
    ...
    check(inputs.encoder_attn_mask.size() == 5, "wan conditioning builds attention mask when rope is off");
    ...
    check(inputs.encoder_attn_mask[2] == 0.0F, "wan conditioning keeps active token positions unmasked");
}
...
void test_text_conditioning_uses_both_prompt_and_null_prompt()
{
    ...
    const bool ok = trtf::diffusion::build_wan_text_conditioning(
        ...
        3,
        error,
        [&encoder_calls](const std::vector<int32_t>& ids, std::vector<float>& embeddings, std::string&) {
            ++encoder_calls;
            embeddings.assign(ids.begin(), ids.end());
            return true;
        },
        [&projector_calls](const std::vector<float>& embeddings, int32_t seq_len, std::vector<float>& projected) {
            ++projector_calls;
    ...
    check(conditioning.text_projected.size() == 4, "wan text conditioning stores prompt projection");
...
void test_conditioning_inputs_skip_attention_mask_when_rope_enabled()
{
    ...
    check(inputs.encoder_attn_mask.empty(),
        "wan rope conditioning omits encoder attention mask");
}
...
void test_text_conditioning_propagates_encoder_failures()
{
    ...
    const bool prompt_fail = trtf::diffusion::build_wan_text_conditioning(
        ...
        [](const std::vector<float>&, int32_t, std::vector<float>&) {
        },
        conditioning);
    ...
    const bool null_fail = trtf::diffusion::build_wan_text_conditioning(
        ...
        2,
        error,
        [&encoder_calls](const std::vector<int32_t>& ids, std::vector<float>& embeddings, std::string& err) {
            ++encoder_calls;
            if (encoder_calls == 2)
            {
