...
void test_blank_advances_frame_without_emit() {
    const auto d = trtf::make_rnnt_greedy_decision(4, 4, 0, 10);
...
void test_nonblank_emits_without_advancing_under_limit() {
    const auto d = trtf::make_rnnt_greedy_decision(2, 4, 0, 10);
...
void test_symbol_limit_advances_after_emit() {
    const auto d = trtf::make_rnnt_greedy_decision(2, 4, 2, 3);
