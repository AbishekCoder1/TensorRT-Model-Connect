...
static void test_detect_empty() {
    auto fmt = trtf::detect_chat_template_format("");
    check(fmt == trtf::ChatTemplateFormat::kNone, "empty -> kNone");
}

static void test_detect_chatml() {
    std::string tpl = "{% for message in messages %}<|im_start|>{{ message.role }}\n{{ "
                      "message.content }}<|im_end|>\n{% endfor %}";
    auto fmt = trtf::detect_chat_template_format(tpl);
    check(fmt == trtf::ChatTemplateFormat::kChatML, "chatml detection");
}
...
static void test_detect_mistral() {
    ...
    auto fmt = trtf::detect_chat_template_format(tpl);
    check(fmt == trtf::ChatTemplateFormat::kMistral, "mistral detection");
}
...
static void test_detect_phi() {
    std::string tpl = "{% for message in messages %}<|user|>\n{{ message.content "
                      "}}<|end|>\n<|assistant|>\n{% endfor %}";
    auto fmt = trtf::detect_chat_template_format(tpl);
    check(fmt == trtf::ChatTemplateFormat::kPhi, "phi detection");
}
...
static void test_detect_gemma() {
    ...
    auto fmt = trtf::detect_chat_template_format(tpl);
    check(fmt == trtf::ChatTemplateFormat::kGemma, "gemma detection");
}
...
static void test_detect_llama3() {
    ...
    auto fmt = trtf::detect_chat_template_format(tpl);
    check(fmt == trtf::ChatTemplateFormat::kLlama3, "llama3 detection");
}

static void test_detect_unknown() {
    auto fmt = trtf::detect_chat_template_format("some random jinja template");
    check(fmt == trtf::ChatTemplateFormat::kNone, "unknown -> kNone");
}

// --- Application tests ---

static void test_apply_none() {
    auto result = trtf::apply_chat_template(trtf::ChatTemplateFormat::kNone, "hello");
    check(result == "hello", "kNone passthrough");
}

static void test_apply_chatml() {
    auto result = trtf::apply_chat_template(trtf::ChatTemplateFormat::kChatML, "What is 2+2?");
...
static void test_apply_chatml_no_thinking() {
    auto result =
        trtf::apply_chat_template(trtf::ChatTemplateFormat::kChatML, "What is 2+2?", false);
    check(result == "<|im_start|>user\nWhat is "
                    "2+2?<|im_end|>\n<|im_start|>assistant\n<think>\n\n</think>\n\n",
...
static void test_apply_mistral() {
    auto result = trtf::apply_chat_template(trtf::ChatTemplateFormat::kMistral, "hello");
    check(result == "[INST] hello [/INST]", "mistral application");
}

static void test_apply_phi() {
    auto result = trtf::apply_chat_template(trtf::ChatTemplateFormat::kPhi, "hello");
    check(result == "<|user|>\nhello<|end|>\n<|assistant|>\n", "phi application");
}

static void test_apply_gemma() {
    auto result = trtf::apply_chat_template(trtf::ChatTemplateFormat::kGemma, "hello");
    check(result == "<start_of_turn>user\nhello<end_of_turn>\n<start_of_turn>model\n",
          "gemma application");
}

static void test_apply_llama3() {
    auto result = trtf::apply_chat_template(trtf::ChatTemplateFormat::kLlama3, "hello");
...
static void test_apply_mistral_no_thinking_ignored() {
    auto result = trtf::apply_chat_template(trtf::ChatTemplateFormat::kMistral, "hello", false);
