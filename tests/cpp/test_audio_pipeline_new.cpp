...
build_mock_encoder(int32_t mel_bins, int32_t mel_len, int32_t enc_out_size)
{
    ...
    auto* const_enc = network->addConstant(nvinfer1::Dims{1, {enc_out_size}},
        nvinfer1::Weights{nvinfer1::DataType::kFLOAT, enc_const.data(), enc_out_size});
    if (!const_enc) return nullptr;
                       ...
                       const std::vector<float>& const_logits)
{
    ...
    auto* token_inp = network->addInput("token_id", nvinfer1::DataType::kINT32,
                                        nvinfer1::Dims{1, {1}});
    auto* mask_inp = network->addInput("attention_mask", nvinfer1::DataType::kFLOAT,
                                       nvinfer1::Dims{1, {mask_size}});

    auto* const_w = network->addConstant(nvinfer1::Dims{1, {vocab_size}},
...
static void test_whisper_transcribe()
{
    ...
    auto encoder = std::make_unique<trtf::TrtModule>(enc_engine.get(), stream);
    ...
    trtf::WhisperPipeline pipeline(
        std::move(encoder), std::move(decoder), std::move(cache),
        wcfg,
        /*hidden_size=*/4, /*num_decoder_layers=*/0,
        std::move(mel_fb),
        /*mel_n_fft=*/400, /*mel_hop_length=*/160,
        /*mel_chunk_length=*/1, /*mel_sampling_rate=*/16000,
        stream);

    check(std::string(pipeline.pipeline_type()) == "WhisperPipeline",
          "whisper pipeline_type");
...
static void test_whisper_constructor_validates_encoder()
{
    ...
    {
        trtf::WhisperPipeline pipeline(
            nullptr, std::move(decoder), std::move(cache),
            wcfg, 4, 0, std::move(mel_fb), 400, 160, 1, 16000, stream);
    }
...
static void test_whisper_with_cross_kv()
{
    ...
    trtf::WhisperPipeline pipeline(
        std::move(encoder), std::move(decoder), std::move(cache),
        wcfg,
        /*hidden_size=*/4, /*num_decoder_layers=*/1,
        std::move(mel_fb),
        /*mel_n_fft=*/400, /*mel_hop_length=*/160,
        /*mel_chunk_length=*/1, /*mel_sampling_rate=*/16000,
        stream);
...
static void test_bark_generate_audio()
{
    ...
    auto semantic = std::make_unique<trtf::TrtModule>(sem_engine.get(), stream);
    auto coarse   = std::make_unique<trtf::TrtModule>(coarse_engine.get(), stream);
    ...
    trtf::BarkPipeline pipeline(
        std::move(semantic), std::move(coarse),
        std::move(sem_cache), std::move(coarse_cache),
        sem_embed, coarse_embed,
        bcfg, stream);

    check(std::string(pipeline.pipeline_type()) == "BarkPipeline",
          "bark pipeline_type");
...
static void test_bark_constructor_validates_embed()
{
    ...
    if (!sem_engine || !coarse_engine) { cudaStreamDestroy(stream); return; }
...
static void test_speech_pipeline_construction()
{
    ...
    auto temporal = std::make_unique<trtf::TrtModule>(temporal_engine.get(), stream);
    ...
    trtf::SpeechPipeline pipeline(
        /*mimi_encoder=*/nullptr,
        std::move(temporal),
        std::move(temporal_cache),
        /*depth_engines=*/{},
        /*depth_cache=*/nullptr,
        /*mimi_decoder=*/nullptr,
        cfg, stream,
        /*subprocess_runner=*/nullptr,
        "test-speech");

    check(std::string(pipeline.pipeline_type()) == "SpeechPipeline",
          "SpeechPipeline: pipeline_type");
    check(std::string(pipeline.model_id()) == "test-speech",
          "SpeechPipeline: model_id");
...
static void test_omni_pipeline_construction()
{
    ...
    auto thinker = std::make_unique<trtf::TrtModule>(thinker_engine.get(), stream);
    ...
    check(std::string(pipeline.pipeline_type()) == "OmniPipeline",
          "OmniPipeline: pipeline_type");
    check(std::string(pipeline.model_id()) == "test-omni",
          "OmniPipeline: model_id");
...
static void test_omni_generate_audio()
{
    ...
    trtf::OmniPipeline pipeline(
        ...
        nullptr, nullptr, nullptr,
        cfg, stream,
        std::make_shared<OmniFixedTokenizer>(),
        "test-omni-gen");
    ...
    check(result.sample_rate == 24000,
          "omni generate_audio: sample_rate = 24000");
...
static void test_whisper_cross_kv_invalid_plan()
{
    ...
    bool ok = trtf::apply_whisper_cross_kv_plan(
        ...
        [](std::size_t, trtf::WhisperCrossKvBufferKind, std::size_t) { return true; },
        error,
        &stats);
