...
void test_encoder_shape_falls_back_to_config_when_cache_shape_is_invalid()
{
    ...
    check(info.encode_codebooks == 4, "speech runtime plan keeps cached codebook count when present");
    check(info.num_frames == 4, "speech runtime plan recomputes frame count when cached shape is invalid");
}
...
void test_encoder_shape_uses_config_when_no_cached_shape_exists()
{
    ...
    check(info.num_frames == 3, "speech runtime plan derives frames from token count and config codebooks");
}
...
void test_encoder_shape_uses_fallback_codebooks_when_cached_shape_is_invalid()
{
    ...
    check(info.num_frames == 3, "speech runtime plan derives frames from fallback encoder codebooks");
}
