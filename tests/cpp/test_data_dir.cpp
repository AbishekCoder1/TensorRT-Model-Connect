...
// Intent:         Source/scripts dir resolution, runtime override
// Preconditions:  set_source_dir_override("") for isolation
...
//   1. Runtime override set via set_source_dir_override(...) (populated by
//      pipeline_factory from the platform.* config namespace).
//   2. TRTF_SOURCE_DIR compile-time define (baked in by CMake).
// Tests reset the override to "" between cases to restore the compile-time
// fallback.
// =============================================================================
...
{
    trtf::set_source_dir_override("");
    const std::string dir = trtf::source_dir();
    check(!dir.empty(), "source_dir not empty");
    check(dir != ".", "source_dir is not fallback dot");
}

static void test_runtime_override()
{
    trtf::set_source_dir_override("/tmp/claude/fake_trtf_root");
    const std::string dir = trtf::source_dir();
    check(dir == "/tmp/claude/fake_trtf_root", "source_dir matches runtime override");
    trtf::set_source_dir_override("");
}
...
static void test_empty_override_uses_compile_default()
{
    trtf::set_source_dir_override("");
    const std::string dir = trtf::source_dir();
    check(dir != "", "empty override falls back to compile-time path");
}
...
int main()
{
    ...
    test_empty_override_uses_compile_default();
