...
static void test_ipipeline_default_virtuals()
{
    ...
    bool threw = false;
    try { pipeline.generate("hello"); } catch (const std::runtime_error&) { threw = true; }
    check(threw, "default generate(string) throws");

    // Default generate(string, image) should throw
    threw = false;
    try { pipeline.generate("hello", nullptr, 0, 0); } catch (const std::runtime_error&) { threw = true; }
    check(threw, "default generate(string,image) throws");

    // Default generate_image should throw
    threw = false;
    try { pipeline.generate_image("prompt"); } catch (const std::runtime_error&) { threw = true; }
    check(threw, "default generate_image throws");

    // Default generate_audio should throw
    threw = false;
    try { pipeline.generate_audio("hello"); } catch (const std::runtime_error&) { threw = true; }
    ...
    threw = false;
    try { pipeline.embed("text"); } catch (const std::runtime_error&) { threw = true; }
    check(threw, "default embed throws");

    // Default rerank should throw
    threw = false;
    try { pipeline.rerank("q", "doc"); } catch (const std::runtime_error&) { threw = true; }
    check(threw, "default rerank throws");

    // Default segment should throw
    threw = false;
    try { pipeline.segment(nullptr, 0, 0); } catch (const std::runtime_error&) { threw = true; }
    check(threw, "default segment throws");

    // Default encode should throw
    threw = false;
    try { pipeline.encode("text"); } catch (const std::runtime_error&) { threw = true; }
    check(threw, "default encode throws");

    // Default solve should throw
    threw = false;
    try { pipeline.solve(nullptr, 0, nullptr, 0); } catch (const std::runtime_error&) { threw = true; }
    check(threw, "default solve throws");

    // Default detect should throw
    threw = false;
    try { pipeline.detect(nullptr, 0, 0); } catch (const std::runtime_error&) { threw = true; }
