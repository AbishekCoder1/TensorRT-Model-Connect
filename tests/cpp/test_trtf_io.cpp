...
// Intent:         trtf::io::write_wav(AudioResult) and read_wav: round-trip,
...
// test_trtf_io.cpp — Unit tests for include/trtf/trtf_io.hpp
//
// Purpose:
//   Validates the inline write_wav(AudioResult, path) and read_wav(path)
//   functions in trtf::io.  These differ from the lower-level trtf::write_wav
//   (in audio_types.h) by operating on AudioResult structs and throwing on
//   errors rather than returning bool.
//
// Dependencies:
//   - trtf/trtf_io.hpp  : trtf::io::write_wav, trtf::io::read_wav
...
static bool test_io_write_read_roundtrip()
{
    ...
    trtf::AudioResult result;
    try
    {
        result = trtf::io::read_wav(path);
    }
    catch (const std::exception& e)
    {
    ...
    {
        std::cerr << "io_write_read_roundtrip: sample_rate mismatch "
                  << result.sample_rate << " vs " << ar.sample_rate << '\n';
        return false;
    }
    if (result.samples.size() != ar.samples.size())
    {
        std::cerr << "io_write_read_roundtrip: sample count mismatch "
                  << result.samples.size() << " vs " << ar.samples.size() << '\n';
        return false;
    }
...
static bool test_io_sample_rate_preserved()
{
    ...
    try { trtf::io::write_wav(ar, path); }
    catch (...) { return false; }

    trtf::AudioResult result;
    try { result = trtf::io::read_wav(path); }
    catch (...) { return false; }
...
static bool test_io_single_sample()
{
    ...
    return result.samples.size() == 1 &&
           std::abs(result.samples[0] - 0.75F) < 1e-6F;
}
...
static bool test_io_write_bad_path_throws()
{
    ...
    try
    {
        trtf::io::write_wav(ar, "/nonexistent/dir/out.wav");
    }
    catch (const std::runtime_error&)
    {
        threw = true;
...
static bool test_io_read_missing_throws()
{
    ...
    try
    {
        trtf::io::read_wav("/nonexistent/path/to/audio.wav");
    }
    catch (const std::runtime_error&)
    {
        threw = true;
...
int main()
{
    ...
    std::cout << "test_trtf_io:" << std::endl;
    ...
    if (all_passed)
    {
        std::cout << "test_trtf_io passed" << std::endl;
        return 0;
    }
    std::cerr << "test_trtf_io FAILED" << std::endl;
    return 1;
}
