...
//   (Wan T2V, FLUX, Z-Image): matrix multiply with optional bias, SiLU and
...
static bool test_silu_values()
{
    ...
    if (data[1] < 1.5F) return false;
    // SiLU(-1) = -1 * sigmoid(-1) ≈ -0.269
    if (data[2] >= 0.0F) return false;
    return true;
}
...
static bool test_gelu_tanh_values()
{
    ...
    if (data[2] >= 0.0F) return false;  // GELU(-1) ≈ -0.159
    return true;
}
...
static bool test_sinusoidal_embedding_zero_value()
{
    ...
    if (!near(emb[0], 1.0F)) return false;
    if (!near(emb[1], 1.0F)) return false;
    // sin(0 * freq) = 0.0 for any freq
    if (!near(emb[2], 0.0F)) return false;
    if (!near(emb[3], 0.0F)) return false;
    return true;
}
...
static bool test_sinusoidal_embedding_range()
{
    ...
    for (float v : emb)
    {
        if (v < -1.001F || v > 1.001F) return false;
    }
    return true;
}
...
static bool test_timestep_mlp_zero_weights()
{
    ...
    for (float v : output)
    {
        if (std::abs(v) > 1e-5F) return false;
    }
    return true;
}
