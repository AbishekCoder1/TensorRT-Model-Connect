...
int main()
{
    ...
    try
    {
        family_dir = trtf_test::make_temp_dir_or_throw("/tmp/trtf_yi_family_XXXXXX");
        write_hf_yi_root(family_dir);
        ...
        {
            std::cerr << "expected yi hf dir to resolve as decoder-definition" << std::endl;
            return 1;
        }
        if (!spec.decoder_model.has_checkpoint)
        {
            std::cerr << "expected yi safetensors bridge to load checkpoint" << std::endl;
            return 1;
        }

        const std::string family = spec.decoder_model.architecture.family;
        if (family.substr(0, 2) != "yi")
        {
            std::cerr << "expected yi architecture family, got " << family << std::endl;
            return 1;
        }
        ...
        {
            std::cerr << "expected yi bridge to load full multi-layer checkpoint" << std::endl;
            return 1;
        }

        if (spec.decoder_model.checkpoint.final_norm.size() != 8)
        {
            std::cerr << "expected yi bridge to load final_norm tensor" << std::endl;
            return 1;
        }

        if (spec.decoder_model.checkpoint.attention_size != 16)
        {
            std::cerr << "expected yi bridge to preserve attention width, got "
...

        {
            ...
            {
                std::cerr << "expected yi q_norm to be empty (no QK norm)" << std::endl;
                return 1;
            }
            if (!layer0.k_norm.empty())
            {
                std::cerr << "expected yi k_norm to be empty (no QK norm)" << std::endl;
                return 1;
            }
        }
