#include "models/llama/registration.h"
#include "models/llama/checkpoint_mapper.h"
#include "trtf/hf_family_registry.h"
#include "model/checkpoint_mapper.h"
#include "runtime/trt/model_runtime_fwd.h"
#include "utils/text_parsers.h"
#include "utils/json_helpers.h"

#include <filesystem>
#include <memory>
#include <string>

namespace trtf {
namespace llama {
namespace {

bool is_llama_model_type(const std::string& model_type)
{
    if (model_type.empty())
    {
        return false;
    }
    const std::string lowered = to_lower_ascii(model_type);
    return starts_with(lowered, "llama");
}

bool has_llama_root_checkpoint(const HfModelMetadata& metadata)
{
    const std::filesystem::path model_dir(metadata.model_dir);
    return std::filesystem::exists(model_dir / "model.safetensors")
        || std::filesystem::exists(model_dir / "model.safetensors.index.json");
}

DecoderModel load_llama_model(const HfModelMetadata& metadata)
{
    DecoderModel model = LoadDecoderModel(metadata.model_dir);

    if (model.max_cache_length > 4096)
    {
        model.max_cache_length = 4096;
    }
    return model;
}

} // namespace

void RegisterLlamaFamily()
{
    RegisterCheckpointMapper("llama", 100, std::make_unique<LlamaCheckpointMapper>());

#if TRTF_HAS_TRT
    RegisterModelRuntime("llama", CreateStandardDecoderRuntime());
    if (!FindModelRuntime("standard-decoder"))
    {
        RegisterModelRuntime("standard-decoder", CreateStandardDecoderRuntime());
    }
#endif

    RegisterHfModelFamily({
        "llama-safetensors",
        100,
        [](const HfModelMetadata& metadata) {
            if (!is_llama_model_type(metadata.model_type))
            {
                return false;
            }
            return has_llama_root_checkpoint(metadata);
        },
        [](const HfModelMetadata& metadata) { return load_llama_model(metadata); },
    });
}

} // namespace llama
} // namespace trtf
