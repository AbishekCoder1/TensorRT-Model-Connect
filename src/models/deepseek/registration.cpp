...
bool is_deepseek_model_type(const std::string& model_type)
{
    if (model_type.empty())
    {
        return false;
    ...
    return starts_with(lowered, "deepseek");
}

bool has_deepseek_root_checkpoint(const HfModelMetadata& metadata)
{
...
DecoderModel load_deepseek_model(const HfModelMetadata& metadata)
{
...
{
    RegisterCheckpointMapper("deepseek", 100, std::make_unique<DeepSeekCheckpointMapper>());

#if TRTF_HAS_TRT
    RegisterTrtGraphBuilder("deepseek", std::make_unique<StandardDecoderGraphBuilder>());
    ...
    RegisterHfModelFamily({
        ...
        [](const HfModelMetadata& metadata) {
            if (!is_deepseek_model_type(metadata.model_type))
            {
                return false;
            }
            return has_deepseek_root_checkpoint(metadata);
        },
        [](const HfModelMetadata& metadata) { return load_deepseek_model(metadata); },
    });
}
