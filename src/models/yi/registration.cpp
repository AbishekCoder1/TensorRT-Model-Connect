...
bool is_yi_model_type(const std::string& model_type)
{
    ...
    return starts_with(lowered, "yi");
}

bool has_yi_root_checkpoint(const HfModelMetadata& metadata)
{
...
void RegisterYiFamily()
{
    ...
    RegisterHfModelFamily({
        ...
        [](const HfModelMetadata& metadata) {
            if (!is_yi_model_type(metadata.model_type))
            {
                return false;
            }
            return has_yi_root_checkpoint(metadata);
        },
