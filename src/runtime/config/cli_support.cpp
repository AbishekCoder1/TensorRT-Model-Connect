...
std::string strip(std::string_view sv)
{
    ...
    while (a < b && is_ws(sv[a])) ++a;
    while (b > a && is_ws(sv[b - 1])) --b;
...
struct Parser {
    ...
    void skip_ws()
    {
        while (i < text.size())
        {
            ...
            if (is_line_comment_start(i)) { skip_line_comment(); continue; }
            break;
        }
    }
    ...
    char decode_string_escape(char esc)
    {
        ...
        for (const auto& entry : table)
            if (entry.first == esc) return entry.second;
    ...
    {
        if (text.compare(i, 4, "true") == 0)  { i += 4; return std::any{true}; }
        if (text.compare(i, 5, "false") == 0) { i += 5; return std::any{false}; }
    ...
    std::any parse_null_literal()
    {
        ...
        if (text.compare(i, 4, "null") == 0) { i += 4; return std::any{}; }
    ...
    bool is_number_continuation(std::size_t at, bool& out_has_dot_or_exp) const
    {
        ...
        if (is_dot_or_exp(ch))  { out_has_dot_or_exp = true; return true; }
        if (is_sign_char(ch))   return prev_was_exp(at);
        return false;
    }
    ...
    std::any parse_number_literal()
    {
        ...
        try
        {
            if (has_dot_or_exp) return std::any{std::stod(num_text)};
    ...
    {
        skip_ws();
        if (i >= text.size()) fail("expected scalar");
        char c = text[i];
        if (c == '"') return std::any{parse_string()};
        if (c == 't' || c == 'f') return parse_bool_literal();
        if (c == 'n') return parse_null_literal();
...
{
    if (v.type() == typeid(std::int32_t)) { os << std::any_cast<std::int32_t>(v); return true; }
    if (v.type() == typeid(std::int64_t)) { os << std::any_cast<std::int64_t>(v); return true; }
    if (v.type() == typeid(int))          { os << std::any_cast<int>(v);          return true; }
    if (v.type() == typeid(double))       { os << std::any_cast<double>(v);       return true; }
    if (v.type() == typeid(float))        { os << std::any_cast<float>(v);        return true; }
    return false;
}

void append_json_scalar(std::ostringstream& os, const std::any& v)
{
    if (!v.has_value()) { os << "null"; return; }
    if (v.type() == typeid(bool))
    {
        os << (std::any_cast<bool>(v) ? "true" : "false");
        return;
    }
    if (try_append_numeric(os, v)) return;
...
const ConfigField* lookup_field_or_throw(
    const SchemaRegistry& registry, const std::string& ns,
    const std::string& field_name, const std::string& origin)
{
    ...
    if (schema == nullptr)
    {
        ...
        throw std::invalid_argument(
            origin + ": unknown namespace '" + ns + "'. Known: " +
            (known.empty() ? "<none registered>" : known));
    }
    for (const auto& cfg_field : schema->fields)
    {
        if (cfg_field.name == field_name) return &cfg_field;
    }
    std::string known;
    ...
    throw std::invalid_argument(
        origin + ": unknown field '" + field_name + "' in namespace '" + ns +
        "'. Known: " + known);
}
...
SetToken parse_set_token(std::string_view token)
{
    ...
    {
        throw std::invalid_argument(
            std::string("--set expects 'ns.field=value' (got ") +
...
std::any coerce_integer(const std::string& raw, const std::string& tag,
...
std::any coerce_floating(const std::string& raw, const std::string& tag,
                         const std::string& where)
{
    try
    {
        ...
        if (consumed != raw.size())
            throw std::invalid_argument("trailing chars");
        if (tag == "double") return std::any{d};
...
std::any coerce_boolean(const std::string& raw, const std::string& where)
{
    ...
    for (auto tok : kTrue)  if (low == tok) return std::any{true};
    for (auto tok : kFalse) if (low == tok) return std::any{false};
    throw std::invalid_argument(
        where + ": expected bool (true/false), got '" + raw + "'");
}
                       ...
                       const std::string& where)
{
    ...
    if (is_integer_tag(tag))  return coerce_integer(raw, tag, where);
    if (is_floating_tag(tag)) return coerce_floating(raw, tag, where);
    if (tag == "bool")        return coerce_boolean(raw, where);
    if (is_string_tag(tag))   return std::any{raw};
    throw std::invalid_argument(
        where + ": schema declares unsupported type_tag '" + type_tag + "' for --set coercion");
}
...
LayeredFileValues load_layered_file(const std::string& path)
{
    ...
    throw std::invalid_argument(
        "--config " + path + ": unsupported extension '" + ext +
        "' (expected .json)");
}
    ...
    const SchemaRegistry& registry)
{
    ...
    for (const auto& token : set_tokens)
    {
        ...
        const ConfigField* field = lookup_field_or_throw(
            registry, st.namespace_name, st.field_name, "--set");
        std::any coerced = coerce_scalar(
            st.raw_value, field->type,
    ...
    const SchemaRegistry& registry)
{
    ...
    contribs.push_back(build_cli_contribution(
...
std::string bundle_to_effective_json(const ConfigBundle& bundle)
{
    ...
    for (std::size_t ni = 0; ni < ns_names.size(); ++ni)
    {
        ...
        for (std::size_t fi = 0; fi < field_names.size(); ++fi)
        {
            ...
            os << "    }";
            if (fi + 1 < field_names.size()) os << ",";
            os << "\n";
        }
        os << "  }";
        if (ni + 1 < ns_names.size()) os << ",";
        os << "\n";
    }
    os << "}\n";
    return os.str();
...
{
    while (p < text.size() && is_json_space(text[p])) ++p;
    return p;
}
