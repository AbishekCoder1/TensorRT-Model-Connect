...
const char* layer_name(Layer layer)
{
    switch (layer)
    {
        case Layer::SchemaDefault:   return "schema_default";
        case Layer::BuildTime:       return "build_time";
        case Layer::BundleDefault:   return "bundle_default";
        case Layer::PlatformProfile: return "platform_profile";
        case Layer::SessionRequest:  return "session_request";
    }
    return "unknown";
}
        ...
        for (const auto& ns_entry : contrib.values)
        {
            ...
            if (schema == nullptr)
            {
                throw std::invalid_argument(
                    std::string("Layer ") + layer_name(contrib.layer) +
            ...
            for (const auto& f_entry : ns_entry.second)
            {
                ...
                if (field == nullptr)
                {
                    throw std::invalid_argument(
                        std::string("Layer ") + layer_name(contrib.layer) +
                        " contributed value for unknown field: " +
                        ns + "." + field_name);
                }
                if (field->allowed_layers.count(contrib.layer) == 0)
                {
                    throw std::invalid_argument(
                        std::string("Layer ") + layer_name(contrib.layer) +
                        " is not permitted to set " + ns + "." + field_name);
                }
                if (field->validator && !field->validator(f_entry.second))
                {
                    throw std::invalid_argument(
                        std::string("Validator rejected value for ") +
                        ns + "." + field_name + " from layer " +
...
const std::any* pick_highest_priority(
    const std::vector<LayerContribution>& contributions,
    ...
    const SchemaRegistry& registry)
{
    ...
    for (const std::string& ns : registry.registered_namespaces())
    {
        ...
        for (const auto& field : schema->fields)
        {
            ...
            const std::any* picked = pick_highest_priority(
                contributions, ns, field.name, source);
            ResolvedValue rv;
            if (picked != nullptr)
            {
                rv.value = *picked;
...
const std::any& ConfigBundle::get_any(
    const std::string& namespace_name, const std::string& field) const
{
    ...
    if (f_it == ns_it->second.end())
    {
        throw std::out_of_range(
            "ConfigBundle: unknown field: " + namespace_name + "." + field);
    }
    return f_it->second.value;
}

Layer ConfigBundle::source_of(
    const std::string& namespace_name, const std::string& field) const
{
    auto ns_it = resolved_.find(namespace_name);
    if (ns_it == resolved_.end())
