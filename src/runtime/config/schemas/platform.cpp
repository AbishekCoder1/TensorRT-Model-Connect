...
bool is_valid_severity(const std::any& v) {
    if (v.type() != typeid(std::string)) return false;
    const auto& s = std::any_cast<const std::string&>(v);
    return s == "INTERNAL_ERROR" || s == "ERROR" || s == "WARNING"
        || s == "INFO" || s == "VERBOSE";
}
} // namespace
...
Schema make_platform_schema()
{
    ...
    return Schema{
        "platform",
        {
            ConfigField{"source_dir", "string", std::any{std::string{}},
                        session, nullptr},
            ConfigField{"trt_log_stderr", "bool", std::any{false},
                        session, nullptr},
            ConfigField{"trt_log_min_severity", "string",
                        std::any{std::string{"INFO"}},
                        session, is_valid_severity},
        },
    };
}
...
struct PlatformRegistrar {
    PlatformRegistrar()
    {
        SchemaRegistry::instance().register_schema(make_platform_schema());
    }
};
