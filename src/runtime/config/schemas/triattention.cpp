...
const std::set<Layer> kBundleAndSession = {
    ...
    Layer::PlatformProfile, Layer::SessionRequest,
};
...
{
    return ConfigField{name, "int32", std::any{default_value}, layers,
                       std::move(validator)};
}
...
ConfigField str_field(const std::string& name, const std::string& default_value,
                      ...
                      std::function<bool(const std::any&)> validator = nullptr)
{
    return ConfigField{name, "string", std::any{default_value}, layers,
                       std::move(validator)};
}
