...
{
    if (num_multimask_outputs <= 0 || result.num_masks <= num_multimask_outputs || result.mask_height <= 0
        || result.mask_width <= 0)
    {
        return result;
    }
    ...
    {
        result.iou_scores = std::vector<float>(
            result.iou_scores.begin() + start_mask,
