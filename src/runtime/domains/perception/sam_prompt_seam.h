    ...
    if (static_cast<int32_t>(point_embed.size()) >= dim)
    {
        for (int32_t i = 0; i < dim; ++i)
        {
            sparse[static_cast<std::size_t>(i)] +=
                point_embed[static_cast<std::size_t>(i)];
        }
    }
    ...
    const std::vector<float>& not_a_point_embed)
{
    ...
    const auto point_emb = encode_sam_point_embedding(
        px,
        py,
        is_foreground,
        image_size,
    ...
    if (static_cast<int32_t>(not_a_point_embed.size()) >= dim)
    {
        for (int32_t i = 0; i < dim; ++i)
        {
            pad_emb[static_cast<std::size_t>(i)] =
                not_a_point_embed[static_cast<std::size_t>(i)];
        }
    }

    std::vector<float> sparse(
        static_cast<std::size_t>(2) * static_cast<std::size_t>(dim));
    std::memcpy(
        sparse.data(),
        point_emb.data(),
        static_cast<std::size_t>(dim) * sizeof(float));
    std::memcpy(
        sparse.data() + static_cast<std::size_t>(dim),
        pad_emb.data(),
        static_cast<std::size_t>(dim) * sizeof(float));
    return sparse;
}
