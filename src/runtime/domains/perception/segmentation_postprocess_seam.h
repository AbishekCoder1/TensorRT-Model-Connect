...
enum class SegmentationPostprocessStatus {
    kOk = 0,
    kInvalidShape = 1,
    kLogitsSizeMismatch = 2,
};

inline bool get_segmentation_logits_expected_size(
    const SegmentationLogitsShape& shape,
...
inline SegmentationPostprocessStatus compute_segmentation_class_map_from_logits(
    const std::vector<float>& logits,
    const SegmentationLogitsShape& shape,
    std::vector<int32_t>& class_map)
{
    std::size_t expected_logits_size = 0;
    if (!get_segmentation_logits_expected_size(shape, expected_logits_size))
    {
        class_map.clear();
        return SegmentationPostprocessStatus::kInvalidShape;
    }

    if (logits.size() != expected_logits_size)
    {
        class_map.clear();
        return SegmentationPostprocessStatus::kLogitsSizeMismatch;
    }
        ...
        for (int32_t x = 0; x < output_w; ++x)
        {
            ...
            for (int32_t c = 0; c < num_classes; ++c)
            {
                const float val = logits[
                    static_cast<std::size_t>(c) * plane_size + pixel_index];
                if (val > best_val)
                {
                    best_val = val;
                    best_class = c;
