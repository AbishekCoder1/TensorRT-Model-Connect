...
inline SpeechWaveformTrimResult trim_speech_waveform_to_generated_frames(
    int32_t sample_rate,
    float frame_rate,
    ...
    std::vector<float>& waveform)
{
    ...
    const int32_t samples_per_frame = static_cast<int32_t>(
        std::lround(static_cast<float>(sample_rate) / frame_rate));
    if (samples_per_frame <= 0)
    {
        return result;
    }

    result.expected_samples = static_cast<std::size_t>(generated_frames)
...
inline SpeechPeakNormalizeResult peak_normalize_speech_waveform(
    std::vector<float>& waveform,
