...
inline std::vector<int32_t> remap_bark_semantic_tokens(
    ...
    const BarkConfig& cfg)
{
    ...
    {
        remapped.push_back(
            token == cfg.semantic_pad_token ? cfg.coarse_semantic_pad_token : token);
    }
    return remapped;
}
...
inline std::vector<int32_t> build_bark_coarse_input_tokens(
    const std::vector<int32_t>& remapped_semantic,
    ...
    const BarkConfig& cfg)
{
    ...
    const int32_t semantic_context_len = std::min(
        semantic_len - semantic_start,
        cfg.max_coarse_input_length);

    std::vector<int32_t> input_tokens;
    input_tokens.reserve(
        static_cast<std::size_t>(cfg.max_coarse_input_length)
        + 1U
        + static_cast<std::size_t>(cfg.max_coarse_history));

    for (int32_t index = semantic_start;
         index < semantic_start + semantic_context_len;
         ++index)
    {
        input_tokens.push_back(remapped_semantic[index]);
    }
    for (int32_t index = semantic_context_len;
         index < cfg.max_coarse_input_length;
         ++index)
    {
    ...
    const int32_t history_start = std::max(
        0,
        total_generated - cfg.max_coarse_history);
...
inline BarkCoarsePlan make_bark_coarse_plan(
    const std::vector<int32_t>& semantic_tokens,
    const BarkConfig& cfg)
{
    BarkCoarsePlan plan;
    plan.remapped_semantic = remap_bark_semantic_tokens(semantic_tokens, cfg);
    plan.total_steps = compute_bark_coarse_steps(
        static_cast<int32_t>(plan.remapped_semantic.size()), cfg);
    if (plan.total_steps > 0)
    {
        plan.num_windows = static_cast<int32_t>(std::ceil(
            static_cast<float>(plan.total_steps) / cfg.sliding_window_len));
    }
    return plan;
}

inline BarkCoarseWindowPlan make_bark_coarse_window_plan(
    const BarkCoarsePlan& coarse_plan,
    ...
    const BarkConfig& cfg)
{
    ...
    plan.generated_this_window = std::min(
        cfg.sliding_window_len,
        coarse_plan.total_steps - plan.start_generated_count);
    if (plan.generated_this_window <= 0)
    {
        plan.generated_this_window = 0;
        return plan;
    }
    plan.input_tokens = build_bark_coarse_input_tokens(
        coarse_plan.remapped_semantic,
        generated_tokens,
        cfg);
    return plan;
}
...
inline BarkCodecPlan make_bark_codec_plan(
    const std::vector<int32_t>& fine_codes,
...
inline std::vector<int32_t> make_bark_codec_input_codes(
    ...
    int32_t actual_frames)
{
    std::vector<int32_t> input_codes(
        static_cast<std::size_t>(target_codebooks) * max_frames,
        0);
    for (int32_t codebook = 0;
         codebook < std::min(source_codebooks, target_codebooks);
         ++codebook)
    {
