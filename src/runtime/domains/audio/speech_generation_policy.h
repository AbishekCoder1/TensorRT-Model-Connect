...
{
    const int32_t nominal_frame_size = (input.frame_rate > 0.0F)
    ...
    if (nominal_frame_size > 0)
    {
        ...
        if (input.input_sample_rate > 0
            && input.input_sample_rate != input.sample_rate)
        {
            effective_input_samples =
                (effective_input_samples * input.sample_rate)
