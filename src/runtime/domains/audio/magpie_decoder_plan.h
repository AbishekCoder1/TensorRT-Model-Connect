...
inline bool should_enable_magpie_cfg(
    const MagpieTTSConfig& config,
    bool has_uncond_cache,
    bool has_uncond_resources,
    bool has_uncond_cross_kv)
{
    return config.cfg_scale > 1.0F
        && has_uncond_cache
        && has_uncond_resources
        && has_uncond_cross_kv;
}

inline bool should_enable_magpie_gpu_greedy(
    bool use_gpu_kernels,
    bool greedy)
{
    return use_gpu_kernels && greedy;
}

inline MagpieDecoderPlan make_magpie_decoder_plan(
    const MagpieTTSConfig& config,
    bool has_uncond_cache,
    bool has_uncond_resources,
    bool has_uncond_cross_kv,
    ...
    int32_t text_length)
{
    ...
    plan.use_cfg = should_enable_magpie_cfg(
        config,
        has_uncond_cache,
        has_uncond_resources,
        has_uncond_cross_kv);
    ...
    plan.use_cross_attn_tracking = has_cross_attn_output
        && has_cross_attn_weights
        && text_length > 0;
    plan.estimated_frames = (!plan.use_cross_attn_tracking && text_length > 0)
        ? static_cast<int32_t>(static_cast<float>(text_length) * 3.0F)
        : 0;
    plan.text_consumed_threshold = std::max(
        static_cast<int32_t>(static_cast<float>(text_length) * 0.9F),
        1);
    return plan;
}
