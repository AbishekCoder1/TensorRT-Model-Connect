...
inline OmniAudioEncodePlan make_omni_audio_encode_plan(
    ...
    int32_t num_frames)
{
    ...
    plan.input_size = static_cast<std::size_t>(num_mel_bins)
        * static_cast<std::size_t>(config.audio_num_frames);
    plan.copy_size = static_cast<std::size_t>(num_mel_bins)
        * static_cast<std::size_t>(plan.actual_frames);
    plan.output_elements = static_cast<std::size_t>(plan.output_frames)
        * static_cast<std::size_t>(plan.embed_dim);
    return plan;
}

inline std::vector<float> build_omni_audio_encoder_input(
    ...
    const OmniAudioEncodePlan& plan)
{
    ...
    if (mel_features != nullptr && plan.copy_size > 0)
    {
        std::memcpy(
            input_padded.data(),
            mel_features,
            plan.copy_size * sizeof(float));
    }
    return input_padded;
}

inline OmniTalkerPlan make_omni_talker_plan(
    std::size_t text_token_count,
    std::size_t hidden_state_count,
    bool has_talker_engine)
{
    OmniTalkerPlan plan;
    plan.should_run_talker = has_talker_engine
        && text_token_count > 0
...
inline OmniCodecPlan make_omni_codec_plan(
    const OmniConfig& config,
    std::size_t codec_token_count)
{
    ...
    plan.n_frames = plan.should_run_codec
        ? static_cast<int32_t>(codec_token_count) / plan.n_codebooks
        : 0;
    plan.should_run_codec = plan.should_run_codec && plan.n_frames > 0;
    return plan;
}

inline OmniTalkerDecodePlan make_omni_talker_decode_plan(
    int32_t n_codebooks,
...
inline int32_t select_omni_codebook_argmax(
...
inline void append_omni_talker_codes_from_logits(
    const std::vector<float>& logits,
    ...
    std::vector<int32_t>& all_codes)
{
    for (int32_t codebook = 0; codebook < plan.n_codebooks; ++codebook)
    {
        ...
        all_codes.push_back(select_omni_codebook_argmax(
            logits,
            offset,
            plan.codebook_size));
    }
...
inline std::vector<int32_t> build_omni_code2wav_input_codes(
