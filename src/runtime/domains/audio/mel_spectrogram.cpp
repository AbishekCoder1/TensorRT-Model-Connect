...
std::vector<float> build_center_padded_audio(
    const float* samples,
    int32_t n_samples,
    int32_t chunk_length_s,
    int32_t sample_rate,
    int32_t n_fft)
{
    ...
    std::memcpy(
        padded.data() + pad_size,
        audio_padded.data(),
        audio_length * sizeof(float));
    return padded;
}

std::vector<float> build_nemo_center_padded_audio(
    const float* samples,
    int32_t n_samples,
    int32_t chunk_length_s,
    int32_t sample_rate,
...
std::vector<float> compute_power_spectrogram(
    ...
    int32_t& n_frames_raw)
{
    ...
    std::vector<float> power(
        static_cast<std::size_t>(n_freq_bins) * n_frames_raw,
        0.0F);
...
std::vector<float> compute_power_spectrogram_with_window(
    const std::vector<float>& padded,
...
std::vector<float> project_power_to_mel(
    ...
    int32_t n_frames_raw)
{
    std::vector<float> mel_spec(
        static_cast<std::size_t>(n_mel_bins) * n_frames_raw,
        0.0F);
...
std::vector<float> trim_last_frame_if_needed(
    std::vector<float> mel_spec,
    int32_t n_mel_bins,
    int32_t n_frames_raw,
...
{
    const std::vector<float> padded = build_center_padded_audio(
        samples, n_samples, chunk_length_s, sample_rate, n_fft);
    ...
    const std::vector<float> power = compute_power_spectrogram(
        padded, n_fft, hop_length, freq_bins, n_frames_raw);
    std::vector<float> mel_spec = project_power_to_mel(
        power, mel_filters, freq_bins, n_mel_bins, n_frames_raw);
    normalize_log_mel_inplace(mel_spec);

    int32_t n_frames_out = 0;
    mel_spec = trim_last_frame_if_needed(
        std::move(mel_spec), n_mel_bins, n_frames_raw, n_frames_out);
