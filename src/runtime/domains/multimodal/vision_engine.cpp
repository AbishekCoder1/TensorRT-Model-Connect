...
bool run_and_copy_outputs(const VisionStepEngine& engine, CudaStream& stream,
    ...
    return run_vision_copy_plan(
        ...
        [&stream]() { return cudaStreamSynchronize(stream.get()) == cudaSuccess; });
}

bool bind_main_outputs(const VisionStepEngine& engine, std::vector<float>& image_features,
                       std::size_t output_bytes, VisionRunState& state, bool allow_host_tensor,
                       std::string& error) {
    if (bind_output_tensor(engine, engine.features_output_name, image_features.data(), output_bytes,
                           state, allow_host_tensor)) {
        return true;
    }
    return fail_with(error, "bind vision output image_features failed");
}

bool bind_pixel_input(const VisionStepEngine& engine, const float* pixel_values,
                      std::size_t pixel_bytes, CudaStream& stream, VisionRunState& state,
                      bool allow_host_tensor, std::string& error) {
    if (bind_input_tensor(engine, engine.pixel_input_name, pixel_values, pixel_bytes, stream, state,
                          allow_host_tensor)) {
        return true;
    }
...
bool bind_deepstack_outputs(const VisionStepEngine& engine, std::size_t output_count,
                            std::size_t output_bytes, VisionRunState& state,
...
bool run_vision_encoder(const VisionStepEngine& engine, const float* pixel_values,
    ...
    const std::size_t output_count =
        vision_output_feature_count(engine.num_output_features, engine.feature_dim);
    ...
    if (!bind_pixel_input(engine, pixel_values, pixel_bytes, stream, state, true, error)) {
        return false;
    }
...
bool run_vision_encoder_with_deepstack(const VisionStepEngine& engine, const float* pixel_values,
    ...
    VisionRunState state;
    if (!bind_pixel_input(engine, pixel_values, pixel_bytes, stream, state, false, error)) {
        return false;
    }
    ...
    if (!bind_deepstack_outputs(engine, output_count, output_bytes, state, deepstack_features,
                                error)) {
        return false;
    }
