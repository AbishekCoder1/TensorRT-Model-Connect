...
TorchMultinomialExecutionPolicy compute_torch_multinomial_execution_policy(int32_t numel) {
    if (numel <= 0) {
        return {};
    }
    ...
    TorchMultinomialExecutionPolicy policy;
    policy.total_threads = static_cast<int32_t>(total_threads);
    policy.counter_offset = counter_offset;
    return policy;
}

void gpu_sparse_torch_multinomial_exact(const int32_t* d_indices, const float* d_probs,
