...
__global__ void triattention_score_candidates_kernel(
    ...
    TriAttentionRopeStyle rope_style, const int32_t* __restrict__ candidate_indices,
    ...
    for (int32_t d = tid; d < half_dim; d += blockDim.x) {
        ...
        if (rope_style == TriAttentionRopeStyle::kInterleaved) {
...
bool launch_score_kernel(const void* d_cache, int32_t kv_dim, int32_t head_dim,
                         TriAttentionRopeStyle rope_style,
    ...
    triattention_score_candidates_kernel<LoadT><<<grid, block, 0, stream>>>(
        d_cache, kv_dim, head_dim, rope_style, d_candidate_indices, candidate_count,
...
bool triattention_score_candidates_gpu(
    ...
    TriAttentionRopeStyle rope_style, const int32_t* d_candidate_indices, int32_t candidate_count,
...
bool triattention_compact_rows_gpu(const void* d_src, void* d_scratch, DType cache_dtype,
