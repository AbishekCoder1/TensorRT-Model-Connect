...
{
    if (!ptr_ || !src || nbytes_ == 0) return false;
    auto err = cudaMemcpyAsync(ptr_, src, nbytes_,
...
{
    if (!ptr_ || !dst || nbytes_ == 0) return false;
...
{
    if (!ptr_ || !other.ptr_ || nbytes_ == 0) return false;
    auto copy_bytes = std::min(nbytes_, other.nbytes_);
    auto err = cudaMemcpyAsync(ptr_, other.ptr_, copy_bytes,
...
void* DeviceTensor::data() { return ptr_; }
const void* DeviceTensor::data() const { return ptr_; }
...
std::size_t DeviceTensor::nbytes() const { return nbytes_; }
