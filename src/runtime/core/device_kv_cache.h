...
class DeviceKvCache {
  public:
    ...
    void update_after_step(const std::vector<CudaBuffer>& present_k,
                           const std::vector<CudaBuffer>& present_v, cudaStream_t stream);

    // Zero all cache buffers and reset cache_length.
    void reset(cudaStream_t stream);

    // Set cache length after bulk-loading (e.g., batched prefill).
    void set_cache_length(int32_t len) { mCacheLength = len; }
    ...
    int32_t cache_state_size() const { return mCacheStateSize; }
...
bool run_decoder_step_device(const DecoderStepEngine& engine, DeviceKvCache& cache,
                             ...
                             const float* input_embed_host = nullptr, int32_t embed_dim = 0,
