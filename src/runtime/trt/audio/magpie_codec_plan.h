...
inline MagpieCodecPlan make_magpie_codec_plan(
    int32_t num_frames,
    int32_t num_cb,
    int32_t max_codec_frames)
{
    ...
    plan.output_elems = static_cast<std::size_t>(plan.max_codec_frames)
        * kMagpieCodecSamplesPerFrame;
    plan.output_bytes = plan.output_elems * sizeof(float);
    plan.valid_samples = static_cast<std::size_t>(plan.padded_frames)
        * kMagpieCodecSamplesPerFrame;
    return plan;
}

inline std::vector<int32_t> build_magpie_codec_input(
    const std::vector<int32_t>& codes,
    int32_t num_cb,
    const MagpieCodecPlan& plan)
{
