...
inline void copy_speech_depth_embedding_row(
...
inline DepthProjectionView make_depth_projection_view(
    ...
    const float* temporal_hidden)
{
    ...
    view.proj_size_per_cb = static_cast<std::size_t>(view.depth_hidden)
        * view.temporal_hidden_dim;
...
inline void apply_depth_projection(
    ...
    float* out_embed)
{
    ...
    {
        float sum = 0.0F;
        const float* proj_row = proj + static_cast<std::size_t>(row) * proj_view.temporal_hidden_dim;
...
inline void seed_depth_text_embedding(
    const SpeechConfig& cfg,
    int32_t text_token,
    ...
    float* depth_embed)
{
    ...
    copy_speech_depth_embedding_row(cfg.depth_text_embedding, emb_offset, depth_hidden, depth_embed);
}

inline void seed_depth_audio_embedding(
    const SpeechConfig& cfg,
    int32_t codebook,
    int32_t prev_token,
    int32_t depth_hidden,
    float* depth_embed)
{
    ...
    copy_speech_depth_embedding_row(cfg.depth_audio_embeddings, emb_offset, depth_hidden, depth_embed);
}

inline void build_depth_input_embedding(
    const SpeechConfig& cfg,
    const DepthProjectionView& proj_view,
    int32_t codebook,
    int32_t text_token,
...
inline int32_t resolve_depth_prev_token(
    int32_t codebook,
