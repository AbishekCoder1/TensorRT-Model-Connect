...
inline EncoderShapeInfo resolve_encoder_shape_without_engine(
    ...
    std::size_t codec_token_count)
{
    ...
    info.encode_codebooks = last_encode_codebooks > 0
    ...
    const bool has_valid_shape = info.encode_codebooks > 0
        ...
        && static_cast<std::size_t>(info.encode_codebooks)
            * static_cast<std::size_t>(info.num_frames) == codec_token_count;
    if (has_valid_shape)
    {
        return info;
    }
...
inline EncoderShapeInfo resolve_encoder_shape_with_fallback_codebooks(
    ...
    int32_t last_encode_frames,
    std::size_t codec_token_count,
    int32_t fallback_encode_codebooks)
{
    ...
    info.encode_codebooks = fallback_encode_codebooks > 0
        ? fallback_encode_codebooks
        : cfg.num_codebooks;
    info.num_frames = (info.encode_codebooks > 0 && codec_token_count > 0)
        ? static_cast<int32_t>(codec_token_count) / info.encode_codebooks
        : 0;
...
inline SpeechGenerationSettings make_speech_generation_settings(
    const SpeechConfig& cfg,
    int32_t hidden,
