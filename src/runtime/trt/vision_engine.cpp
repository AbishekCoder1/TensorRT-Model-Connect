    ...
    std::string& error)
{
    ...
    auto bind_input = [&](const std::string& name, const void* host_ptr, std::size_t bytes) -> bool {
        ...
        if (cudaMemcpyAsync(buffer->data(), host_ptr, bytes, cudaMemcpyHostToDevice, stream.get()) != cudaSuccess)
            return false;
        if (!engine.context->setTensorAddress(name.c_str(), buffer->data())) return false;
        device_buffers.push_back(std::move(buffer));
        return true;
    };
    ...
    // Discover and bind deepstack outputs
    deepstack_features.clear();
    for (int32_t i = 0; ; ++i)
    {
        std::string ds_name = "deepstack_features_" + std::to_string(i);
        if (!has_io_tensor(*engine.engine, ds_name)) break;

        deepstack_features.emplace_back(output_count, 0.0F);
        if (!bind_output(ds_name, deepstack_features.back().data(), output_bytes))
        {
            return fail("bind deepstack output " + ds_name + " failed");
        }
    }
