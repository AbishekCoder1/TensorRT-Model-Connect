...
struct SamImageEncodePlan {
    ...
    {
        return !pixel_values.empty() && rescaled_width > 0 && rescaled_height > 0 && original_width > 0
            && original_height > 0;
    }
};

inline float sample_image_channel_bilinear(
    ...
    float y)
{
    ...
    const auto idx = [stride = image.channels, width = image.width](int32_t yy, int32_t xx, int32_t cc) {
...
inline SamImageEncodePlan build_sam_image_encode_plan(
    const runtime::adapters::io::DecodedImage& image,
    const SamConfig& config)
{
    ...
    plan.rescaled_height = static_cast<int32_t>(std::round(static_cast<float>(image.height) * scale));
