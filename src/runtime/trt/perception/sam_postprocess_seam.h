...
inline std::vector<float> resize_mask_bilinear(
    const std::vector<float>& mask,
    int32_t src_w,
    int32_t src_h,
    int32_t dst_w,
    int32_t dst_h)
{
    ...
    std::vector<float> resized(static_cast<std::size_t>(dst_w) * static_cast<std::size_t>(dst_h), 0.0F);
    ...
    for (int32_t y = 0; y < dst_h; ++y)
    {
        ...
        for (int32_t x = 0; x < dst_w; ++x)
        {
            ...
            resized[static_cast<std::size_t>(y) * static_cast<std::size_t>(dst_w)
                    + static_cast<std::size_t>(x)] = sample_mask_bilinear(mask, src_w, src_h, src_x, src_y);
        }
    }
    return resized;
}

inline bool has_valid_sam_postprocess_request(
    const SamResult& result,
    int32_t image_size,
    int32_t rescaled_w,
    int32_t rescaled_h,
    int32_t original_w,
...
inline std::vector<float> extract_sam_mask(
    const SamResult& result,
    int32_t mask_index)
{
    const auto mask_size =
        static_cast<std::size_t>(result.mask_width) * static_cast<std::size_t>(result.mask_height);
    const auto begin = result.masks.begin()
        + static_cast<std::ptrdiff_t>(mask_index) * static_cast<std::ptrdiff_t>(mask_size);
    return std::vector<float>(begin, begin + static_cast<std::ptrdiff_t>(mask_size));
}

inline std::vector<float> crop_rescaled_sam_mask(
    const std::vector<float>& upsampled,
    int32_t image_size,
    int32_t rescaled_w,
    int32_t rescaled_h)
{
    std::vector<float> cropped(
        static_cast<std::size_t>(rescaled_w) * static_cast<std::size_t>(rescaled_h),
        0.0F);
    for (int32_t y = 0; y < rescaled_h; ++y)
    {
        const auto* src_row = upsampled.data()
            + static_cast<std::size_t>(y) * static_cast<std::size_t>(image_size);
        auto* dst_row = cropped.data()
            + static_cast<std::size_t>(y) * static_cast<std::size_t>(rescaled_w);
        std::copy_n(src_row, rescaled_w, dst_row);
    }
    return cropped;
}

inline std::vector<float> postprocess_single_sam_mask(
    const SamResult& result,
    int32_t mask_index,
    int32_t image_size,
    int32_t rescaled_w,
    ...
    int32_t original_h)
{
    auto upsampled = resize_mask_bilinear(
        extract_sam_mask(result, mask_index),
        result.mask_width,
        result.mask_height,
        image_size,
        image_size);
    return resize_mask_bilinear(
        crop_rescaled_sam_mask(upsampled, image_size, rescaled_w, rescaled_h),
        rescaled_w,
        rescaled_h,
        original_w,
        original_h);
...
inline SamResult postprocess_sam_result(
    SamResult result,
    int32_t image_size,
    int32_t rescaled_w,
    int32_t rescaled_h,
    ...
    int32_t original_h)
{
    if (!has_valid_sam_postprocess_request(
            result, image_size, rescaled_w, rescaled_h, original_w, original_h))
    {
        return result;
    }
    ...
    processed.reserve(
        static_cast<std::size_t>(result.num_masks) * static_cast<std::size_t>(original_w)
        * static_cast<std::size_t>(original_h));

    for (int32_t mask_index = 0; mask_index < result.num_masks; ++mask_index)
    {
        auto restored = postprocess_single_sam_mask(
            result, mask_index, image_size, rescaled_w, rescaled_h, original_w, original_h);
