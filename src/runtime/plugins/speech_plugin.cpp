...
class SpeechPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        auto temporal_loaded = load_trt_module_from_plan(find_section(ctx.bundle, "engine_plan"),
                                                         "speech temporal", shared_stream);
        ...
        auto mimi_encoder = extract_optional_module(find_section(ctx.bundle, "mimi_encoder_plan"),
                                                    "speech mimi_encoder", shared_stream);
        auto mimi_decoder = extract_optional_module(find_section(ctx.bundle, "mimi_decoder_plan"),
                                                    "speech mimi_decoder", shared_stream);
