...
ImageResult convert_vae_output(
    const float* raw,
    int32_t h_out,
    int32_t w_out)
{
...
bool ZImagePipeline::run_text_encoder(
    ...
    std::vector<float>& text_embeddings)
{
    ...
    std::memcpy(text_embeddings.data(), te_out.data,
...
bool ZImagePipeline::run_denoiser(
...
void ZImagePipeline::project_caption(
    ...
    std::vector<float>& projected) const
{
    ...
    for (int32_t s = 0; s < text_seq; ++s) {
        ...
        for (int32_t d = 0; d < te_dim; ++d) {
            out_row[d] = row[d] * inv_rms *
                z_weights_.cap_norm_weight[static_cast<std::size_t>(d)];
        }
    }
    ...
    cpu_matmul_bias(normed.data(), z_weights_.cap_proj_weight.data(),
                    z_weights_.cap_proj_bias.data(), projected.data(),
                    text_seq, te_dim, dit_dim);
...
void ZImagePipeline::compute_3d_rope(
    ...
    auto encode_pos = [&](float* cos_row, float* sin_row,
        ...
        for (int32_t i = 0; i < kRopeDimT / 2; ++i) {
            const float freq = 1.0F / std::pow(
                kRopeTheta, 2.0F * static_cast<float>(i) / static_cast<float>(kRopeDimT));
        ...
        for (int32_t i = 0; i < kRopeDimH / 2; ++i) {
            const float freq = 1.0F / std::pow(
                kRopeTheta, 2.0F * static_cast<float>(i) / static_cast<float>(kRopeDimH));
        ...
        for (int32_t i = 0; i < kRopeDimW / 2; ++i) {
            const float freq = 1.0F / std::pow(
                kRopeTheta, 2.0F * static_cast<float>(i) / static_cast<float>(kRopeDimW));
...
void ZImagePipeline::patchify_2d(
    const std::vector<float>& latents,
    ...
    std::vector<float>& patches) const
{
    ...
    patches.resize(
        static_cast<std::size_t>(num_patches_val) * static_cast<std::size_t>(patch_dim));
...
void ZImagePipeline::unpatchify_2d(
    ...
    const GenerateConfig& cfg)
{
    ...
    const int32_t num_inference_steps = resolve_requested_steps(
        cfg.num_steps, config_.num_inference_steps, true);
    const float guidance_scale = resolve_requested_guidance(
        cfg.guidance_scale, config_.guidance_scale);
    ...
    std::cerr << "[z-image] Caption: " << cap_ori_len << " actual tokens, "
              << cap_padded_len << " padded (SEQ_MULTI_OF=" << kSeqMultipleOf << ")\n";
    ...
    compute_3d_rope(cap_padded_len, layout.num_patches,
    ...
    for (int32_t step = 0; step < num_inference_steps; ++step) {
        ...
        const float t_for_embedding = 1000.0F - raw_timestep;
        {
            ...
            cpu_matmul_bias(sinusoidal.data(), z_weights_.t_embedder_mlp_0_weight.data(),
                            z_weights_.t_embedder_mlp_0_bias.data(), h1.data(),
                            1, freq_dim, mid_dim);
            ...
            cpu_matmul_bias(h1.data(), z_weights_.t_embedder_mlp_2_weight.data(),
                            z_weights_.t_embedder_mlp_2_bias.data(), temb.data(),
                            1, mid_dim, freq_dim);
        }
        ...
        if (!run_denoiser(hidden, caption_projected, temb,
                          rope_cos, rope_sin, denoiser_output)) {
