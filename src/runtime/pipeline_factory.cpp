...
std::string normalize_legacy_strategy(
    const std::string& strategy, const std::string& config_text)
{
    if (strategy == "text_to_audio")
    {
        return json_field_is_truthy(config_text, "magpie_tts")
            ? "text_to_audio_magpie" : "text_to_audio_bark";
    }
    if (strategy == "diffusion")
    {
        auto bt = json_field_substr(config_text, "diffusion_backend_type");
        if (bt.find("flux") != std::string::npos) return "diffusion_flux";
        if (bt.find("z_image") != std::string::npos) return "diffusion_zimage";
        if (bt.find("pixart") != std::string::npos) return "diffusion_pixart";
    ...
    const std::string& bundle_path, const std::string& hf_python)
{
    ...
    if (strategy.empty()) strategy = "decoder_kv_cache";
    ...
    std::cerr << "[trtf] Pipeline loaded (strategy=" << strategy
