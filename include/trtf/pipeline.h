...
public:
    ...
    {
        (void) image_pixels; (void) image_height; (void) image_width;
    ...
    {
        (void) prompt; (void) cfg;
        throw std::runtime_error(std::string(pipeline_type()) + " does not support generate_image()");
    }

    virtual ImageResult generate_image(const TextEmbedding& emb, const GenerateConfig& cfg = {})
    {
        (void) emb; (void) cfg;
        throw std::runtime_error(std::string(pipeline_type()) + " does not support generate_image(TextEmbedding)");
    }
    ...
    {
        (void) prompt; (void) cfg;
        throw std::runtime_error(std::string(pipeline_type()) + " does not support generate_audio()");
    }
    ...
    virtual TextResult transcribe(const float* audio_samples, int32_t num_samples,
                                  int32_t max_tokens = 224,
                                  int32_t input_sample_rate = 0)
    {
        (void) audio_samples; (void) num_samples; (void) max_tokens;
    ...
    virtual AudioResult speak(const float* audio_in, int32_t num_samples,
                              ...
                              int32_t input_sample_rate = 0)
    {
        (void) audio_in; (void) num_samples; (void) cfg;
        (void) input_sample_rate;
    ...
    {
        (void) pixels; (void) height; (void) width;
                                  ...
                                  const float* trunk_input, int32_t trunk_len)
    {
        ...
        (void) trunk_input; (void) trunk_len;
    ...
    {
        (void) pixels; (void) height; (void) width; (void) conf_threshold;
