...
```
RegisterBuiltinHfModelFamilies()          ← auto-generated by CMake
...
The function body is auto-generated at configure time from `cmake/family_dispatch.cpp.in`. CMake discovers family directories by globbing `src/models/*/registration.h`.
...
### Registry 3: Model Runtime (`RegisterModelRuntime`)

**Interface**: `IModelRuntime { build_engine(), create_state(), run_step() }`

Each model family owns its full forward pass: graph construction, state creation, and per-step execution. `IModelRuntime` replaces the previous `ITrtGraphBuilder` registry, giving families control over not just the graph but also the runtime loop.

**Factory helpers** (no public base classes — true DI):
- `CreateStandardDecoderRuntime()` — returns an anonymous `IModelRuntime` that uses `StandardDecoderGraphBuilder` + KV-cache state/step. Used by Qwen, LLaMA, Mistral, Gemma.
- `CreateKvCacheRuntime(engine_factory)` — custom engine builder + standard KV-cache state/step. For MoE or custom graph families.
...
HF Model Directory (config.json + model.safetensors + tokenizer.json)
  ...
  └──[tokenizer.json]──→ HfPythonTokenizer (subprocess bridge)
                    ...
                    TRT Pipeline
                           ...
                           ├─→ INetworkDefinition (Registry 3: Model Runtime → graph builder)
...
`RegisterBuiltinHfModelFamilies()` is called via `std::call_once` only when the HF family registry is actually needed. The function body is auto-generated by CMake from `cmake/family_dispatch.cpp.in`. This means:
- Adding a new family requires zero edits to shared files (just create files + re-run cmake)
