...
User: trtf_create_pipeline("QWEN3", TRTF_FORCE_TRT)
  ...
  │           │
  │           ├── RegisterBuiltinHfModelFamilies() [once, auto-generated by CMake]
  ...
  │                 │
  │                 ├── FindModelRuntime("qwen3") → IModelRuntime (via CreateStandardDecoderRuntime)
  │                 │
  │                 └── CreateTrtBackendWithRuntime(tokenizer, model, runtime)
  ...
  │                       │
  │                       ├── runtime.build_engine(definition, logger)
  │                       │     → delegates to StandardDecoderGraphBuilder
  │                       │     → creates ICudaEngine
  │                       │
  │                       └── TrtBackend (holds engine, uses runtime for state + step)
  │
