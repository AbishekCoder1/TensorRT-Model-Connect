...
| **Section 7.4.3**: Restricted coupling | Coupling between components shall be restricted and documented. | Implemented | Pipeline factory (`src/runtime/registry/pipeline_factory.cpp`) delegates to self-registered plugins via `PipelineRegistry`. Each plugin in `src/runtime/plugins/` is self-contained. Pipelines depend only on `TrtModule` and `KvCache`/`RecurrentState` abstractions. Python build package (`trtf_build/`) is decoupled from C++ runtime. | None for QM |
