...
def main():
    ...
    parser.add_argument("--trtf-binary",
                        help="Path to compiled trtf binary for C++ benchmark pass "
                             "(e.g. ./build/trtf).  Skipped when not provided.")
    parser.add_argument("--hf-python",
                        help="Python interpreter path passed to trtf binary "
                             "(only used with --trtf-binary)")
    ...
    if args.trtf_binary and args.bundle:
        ...
        if cpp_res is None:
            print("[profile] C++ pass failed; continuing without it.",
                  file=sys.stderr)
    elif args.trtf_binary and not args.bundle:
        print("[profile] --trtf-binary requires --bundle (no on-the-fly engine "
    ...
    if getattr(args, "nsight", False) and args.trtf_binary and args.bundle:
        ...
        print("[profile] --nsight requires --trtf-binary and --bundle; skipping.",
              file=sys.stderr)
