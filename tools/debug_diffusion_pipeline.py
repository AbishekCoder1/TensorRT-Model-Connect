...
def compute_timestep_embedding_np(timestep: float, pp: dict, freq_dim: int = 256):
    """Compute timestep embedding using preprocessor weights (matching C++)."""
    half = freq_dim // 2
...
def project_text_np(text_emb: np.ndarray, pp: dict):
    """Project T5 text embeddings to DiT dimension (matching C++)."""
...
def step6_3d_rope(atol: float) -> bool:
    ...
    print(f"  Small (2x16x16 -> 128 patches):")
