...
def build_vision_engine_from_onnx(
    ...
    *,
    ...
    if verbose:
        print(f"[trtf-build] Building vision TRT engine from ONNX "
...
def trace_hf_vision_encoder(
    ...
    *,
    ...
    if verbose:
        print(f"[trtf-build] Tracing vision encoder to ONNX "
    ...
    if verbose:
        print(f"[trtf-build] ONNX export done ({len(onnx_bytes) / 1024:.0f} KB)",
              file=sys.stderr)
