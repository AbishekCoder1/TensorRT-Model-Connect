...
def profile_env_var(profile_name: str) -> str:
    ...
    return f"TRTF_PYTHON_PROFILE_{_profile_token(profile_name)}_PYTHON"
...
def profile_env_var_candidates(profile_name: str) -> tuple[str, ...]:
    ...
    return (
        f"TRTF_PYTHON_PROFILE_{token}_PYTHON",
        f"TRTF_E2E_PROFILE_{token}_PYTHON",
    )
...
def load_python_profile_registry() -> dict[str, Any]:
    """Load the declarative profile registry bundled with trtf-build."""
...
def _write_base_site_packages_overlay(base_python: str, profile_python: str) -> None:
    ...
    overlay_file = Path(profile_paths[0]) / "trtf_base_python_overlay.pth"
