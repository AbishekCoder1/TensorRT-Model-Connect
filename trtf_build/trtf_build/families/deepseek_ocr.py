...
class DeepSeekOCRPlugin:
    ...
    def build_engine(
        ...
        # -----------------------------------------------------------
        if verbose:
            print(f"[trtf-build] Building DeepSeek-OCR TRT engine "
...
def _build_deepseek_ocr_vision_engine(
    ...
    Output: image_features [257, 1280] float32 (256 projected + 1 view_sep)
    """
    print("[trtf-build] Building DeepSeek-OCR-2 vision engine (native TRT) ...",
          file=sys.stderr)
    ...
    if verbose:
        print(f"[trtf-build] Building vision TRT engine "
