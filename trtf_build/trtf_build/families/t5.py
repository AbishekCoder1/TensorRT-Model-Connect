...
class T5Plugin:
    ...
    def build_engine(self, config, weights, max_cache_length, *, verbose=False, debug_layer_outputs=False):
        ...
        if verbose:
            print(f"[trtf-build] T5 decoder ({dl}L, h={h}, heads={nh}, d_kv={dkv}, ffn={dff}, cache={max_cache_length})", file=sys.stderr)
...
def _build_t5_encoder(config, weights, *, verbose=False):
    ...
    if verbose:
        print(f"[trtf-build] T5 encoder ({el}L, h={h}, heads={nh}, seq={msl})", file=sys.stderr)
    plan = builder.build_serialized_network(network, tc)
    if plan is None:
