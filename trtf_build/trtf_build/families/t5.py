...
class T5Plugin:
    ...
    def build_engine(self, config, weights, max_cache_length, *, verbose=False, debug_layer_outputs=False):
        ...
        dl = weights["_dec_layers"]; nh = weights["_num_heads"]; dkv = weights["_d_kv"]
        dff = weights["_d_ff"]; h = weights["_hidden"]; vs = weights["_vocab_size"]
        nb = weights["_num_buckets"]; md = weights["_max_distance"]; eps = weights["_layer_norm_eps"]
        hd = dkv; asz = nh * hd; aw = max_cache_length + 1; msp = max_cache_length
        ...
        dsrb = weights.get("dec_self_rel_attn_bias"); dxrb = weights.get("dec_cross_rel_attn_bias")
        if debug_layer_outputs: _mark_debug_output(network, hs, "debug_embed")
        pk_out, pv_out = [], []
        for li in range(dl):
            pfx = f"layer.{li}"
            ...
            hs = r["hidden"]; pk_out.append(r["present_k"]); pv_out.append(r["present_v"])
            if debug_layer_outputs: _mark_debug_output(network, hs, f"debug_hidden_{li}")
        ...
        for i in range(dl):
            ...
            network.mark_output(pk_out[i]); network.mark_output(pv_out[i])
        if verbose: print(f"[trtf-build] T5 decoder ({dl}L, h={h}, heads={nh}, d_kv={dkv}, ffn={dff}, cache={max_cache_length})", file=sys.stderr)
        plan = builder.build_serialized_network(network, tc)
        if plan is None: raise RuntimeError("TRT T5 decoder build failed")
        return bytes(plan)
    ...
    def get_vl_config(self, config):
        raw = config.raw; nl = raw.get("num_layers", config.num_hidden_layers)
...
def _build_t5_encoder(config, weights, *, verbose=False):
    el = weights["_enc_layers"]; nh = weights["_num_heads"]; dkv = weights["_d_kv"]
    dff = weights["_d_ff"]; h = weights["_hidden"]; vs = weights["_vocab_size"]
    nb = weights["_num_buckets"]; md = weights["_max_distance"]; eps = weights["_layer_norm_eps"]
    msl = weights.get("_max_cache_length", config.raw.get("n_positions", 512)); hd = dkv; asz = nh * hd
    logger = trt.Logger(trt.Logger.VERBOSE if verbose else trt.Logger.WARNING)
    builder = trt.Builder(logger); network = builder.create_network()
    tc = builder.create_builder_config()
    tc.set_memory_pool_limit(trt.MemoryPoolType.WORKSPACE, 1 << 30)
    ...
    if verbose: print(f"[trtf-build] T5 encoder ({el}L, h={h}, heads={nh}, seq={msl})", file=sys.stderr)
    plan = builder.build_serialized_network(network, tc)
    if plan is None: raise RuntimeError("TRT T5 encoder build failed")
    return bytes(plan)
...
def _add_t5_enc_sa(network, hidden, wq, wk, wv, wo, h, nh, hd, asz, sl, rpb=None, attn_mask=None):
    ...
    qh = network.add_shuffle(q); qh.reshape_dims = (sl, nh, hd); qh.second_transpose = trt.Permutation([1,0,2])
    kh = network.add_shuffle(k); kh.reshape_dims = (sl, nh, hd); kh.second_transpose = trt.Permutation([1,0,2])
    vh = network.add_shuffle(v); vh.reshape_dims = (sl, nh, hd); vh.second_transpose = trt.Permutation([1,0,2])
    sc = network.add_matrix_multiply(qh.get_output(0), trt.MatrixOperation.NONE, kh.get_output(0), trt.MatrixOperation.TRANSPOSE)
    scores = sc.get_output(0)
    if rpb is not None:
        scores = network.add_elementwise(scores, rpb, trt.ElementWiseOperation.SUM).get_output(0)
    if attn_mask is not None:
        scores = network.add_elementwise(scores, attn_mask, trt.ElementWiseOperation.SUM).get_output(0)
    sm = network.add_softmax(scores); sm.axes = 1 << 2
    ctx = network.add_matrix_multiply(sm.get_output(0), trt.MatrixOperation.NONE, vh.get_output(0), trt.MatrixOperation.NONE)
    cf = network.add_shuffle(ctx.get_output(0)); cf.first_transpose = trt.Permutation([1,0,2]); cf.reshape_dims = (sl, asz)
    return graph_ops.add_matmul_rhs_constant(network, cf.get_output(0), asz, h, wo)
...
def _add_t5_decoder_layer(*, network, hidden, cache_k, cache_v, cross_k, cross_v, attention_mask, position_id, eps_tensor, weights, prefix, hidden_size, num_heads, head_dim, attention_size, ffn_dim, max_cache_length, max_source_positions, dec_self_rel_bias, dec_cross_rel_bias, num_buckets, max_distance, enc_mask=None):
    ...
    aw = max_cache_length + 1; msp = max_source_positions
    ...
    kr = network.add_shuffle(k); kr.reshape_dims = (1, asz)
    vr = network.add_shuffle(v); vr.reshape_dims = (1, asz)
    ...
    qh = network.add_shuffle(q); qh.reshape_dims = (nh, 1, hd)
    kh = network.add_shuffle(ak.get_output(0)); kh.reshape_dims = (aw, nh, hd); kh.second_transpose = trt.Permutation([1,0,2])
    vh = network.add_shuffle(av.get_output(0)); vh.reshape_dims = (aw, nh, hd); vh.second_transpose = trt.Permutation([1,0,2])
    sc = network.add_matrix_multiply(qh.get_output(0), trt.MatrixOperation.NONE, kh.get_output(0), trt.MatrixOperation.TRANSPOSE)
    scores = sc.get_output(0)
    if dec_self_rel_bias is not None:
        ...
        scores = network.add_elementwise(scores, br.get_output(0), trt.ElementWiseOperation.SUM).get_output(0)
    m3 = network.add_shuffle(attention_mask); m3.reshape_dims = (1, 1, aw)
    ma = network.add_elementwise(scores, m3.get_output(0), trt.ElementWiseOperation.SUM)
    ...
    cf = network.add_shuffle(ct.get_output(0)); cf.reshape_dims = (1, asz)
    sa = graph_ops.add_matmul_rhs_constant(network, cf.get_output(0), asz, h, weights[f"{prefix}.w_o"])
    ...
    cqh = network.add_shuffle(cq); cqh.reshape_dims = (nh, 1, hd)
    ckh = network.add_shuffle(ckp); ckh.reshape_dims = (msp, nh, hd); ckh.second_transpose = trt.Permutation([1,0,2])
    cvh = network.add_shuffle(cvp); cvh.reshape_dims = (msp, nh, hd); cvh.second_transpose = trt.Permutation([1,0,2])
    cs = network.add_matrix_multiply(cqh.get_output(0), trt.MatrixOperation.NONE, ckh.get_output(0), trt.MatrixOperation.TRANSPOSE)
    xscores = cs.get_output(0)
    if dec_cross_rel_bias is not None:
        ...
        xscores = network.add_elementwise(xscores, xbr.get_output(0), trt.ElementWiseOperation.SUM).get_output(0)
    if enc_mask is not None:
        xscores = network.add_elementwise(xscores, enc_mask, trt.ElementWiseOperation.SUM).get_output(0)
    csm = network.add_softmax(xscores); csm.axes = 1 << 2
    cc = network.add_matrix_multiply(csm.get_output(0), trt.MatrixOperation.NONE, cvh.get_output(0), trt.MatrixOperation.NONE)
    ccf = network.add_shuffle(cc.get_output(0)); ccf.reshape_dims = (1, asz)
    ca = graph_ops.add_matmul_rhs_constant(network, ccf.get_output(0), asz, h, weights[f"{prefix}.cross_w_o"])
...
def _make_t5_causal_buckets(ml, nb=32, md=128):
    cp = np.arange(ml, dtype=np.int32)[:, None]; mp = np.arange(ml, dtype=np.int32)[None, :]
    n = np.maximum(-(mp - cp), 0); me = nb // 2; is_s = n < me
...
def _make_t5_cross_buckets(ql, kl, nb=32, md=128):
    cp = np.arange(ql, dtype=np.int32)[:, None]; mp = np.arange(kl, dtype=np.int32)[None, :]
    n = np.maximum(-(mp - cp), 0); me = nb // 2; is_s = n < me
    nc = np.maximum(n.astype(np.float32), 1)
    vl = me + (np.log(nc / me) / np.log(md / me) * (nb - me)).astype(np.int32)
    return np.where(is_s, n, np.minimum(vl, nb - 1)).astype(np.int32)
