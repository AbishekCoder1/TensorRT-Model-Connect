...
class GptOssPlugin:
    ...
    def load_weights(
        ...
        print("[trtf-build] Loading GPT-OSS model (MXFP4 auto-dequant) ...",
    ...
    def build_engine(
        ...
        # -----------------------------------------------------------
        if verbose:
            print(f"[trtf-build] Building GPT-OSS MoE TRT engine "
...
def _add_gpt_oss_attention(
    ...
    *,
    ...
    if sinks is not None:
        ...
        # No sinks: standard softmax
        softmax = network.add_softmax(masked.get_output(0))
        softmax.axes = 1 << 2
        attn_probs = softmax.get_output(0)
