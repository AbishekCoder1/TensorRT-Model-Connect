...
def _add_seq_layer_norm(network, inp, hidden_size, gamma, beta, eps):
    ...
    sq = network.add_elementwise(centered.get_output(0), centered.get_output(0), trt.ElementWiseOperation.PROD)
    var = network.add_reduce(sq.get_output(0), trt.ReduceOperation.AVG, 1 << 1, keep_dims=True)
    denom_in = network.add_elementwise(var.get_output(0), eps_t, trt.ElementWiseOperation.SUM)
    sqrt_l = network.add_unary(denom_in.get_output(0), trt.UnaryOperation.SQRT)
    recip = network.add_unary(sqrt_l.get_output(0), trt.UnaryOperation.RECIP)
    normalized = network.add_elementwise(centered.get_output(0), recip.get_output(0), trt.ElementWiseOperation.PROD)
    scaled = network.add_elementwise(normalized.get_output(0), gamma_t, trt.ElementWiseOperation.PROD)
...
def _build_deberta_encoder_engine(config, weights, max_seq_length, *, verbose=False):
    ...
    if verbose:
        print(f"[trtf-build] Building DeBERTa encoder ({num_layers} layers, hidden={hidden}, seq={max_seq_length})", file=sys.stderr)
