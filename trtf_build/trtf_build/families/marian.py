    ...
    def build_engine(self, config: ModelConfig, weights: WeightDict,
        ...
        for layer_idx in range(dec_layers):
            ...
            result = _add_marian_decoder_layer(
                ...
                weights=weights, prefix=prefix, hidden_size=hidden,
        ...
        if verbose:
            print(f"[trtf-build] Building Marian decoder ({dec_layers}L, h={hidden}, heads={dec_heads}, ffn={dec_ffn}, cache={max_cache_length})", file=sys.stderr)
...
def _build_marian_encoder(config, weights, *, verbose=False):
    ...
    pos_embed_table = graph_ops.add_constant(network, pos_embed_np.shape, pos_embed_np)
    ...
    # Reshape attention mask [max_pos] -> [1, 1, max_pos] for broadcasting
    # with attention scores [num_heads, max_pos, max_pos]
    enc_mask_3d = network.add_shuffle(attention_mask)
    enc_mask_3d.reshape_dims = (1, 1, max_pos)
    ...
    for li in range(enc_layers):
        ...
        q_h = network.add_shuffle(q); q_h.reshape_dims = (max_pos, enc_heads, head_dim); q_h.second_transpose = trt.Permutation([1, 0, 2])
        k_h = network.add_shuffle(k); k_h.reshape_dims = (max_pos, enc_heads, head_dim); k_h.second_transpose = trt.Permutation([1, 0, 2])
        v_h = network.add_shuffle(v); v_h.reshape_dims = (max_pos, enc_heads, head_dim); v_h.second_transpose = trt.Permutation([1, 0, 2])
        ...
        # Apply encoder attention mask (0.0 for real, -1e9 for padding)
        masked = network.add_elementwise(scaled.get_output(0), enc_mask_3d.get_output(0), trt.ElementWiseOperation.SUM)
        softmax = network.add_softmax(masked.get_output(0)); softmax.axes = 1 << 2

        context = network.add_matrix_multiply(softmax.get_output(0), trt.MatrixOperation.NONE, v_h.get_output(0), trt.MatrixOperation.NONE)
        ctx_flat = network.add_shuffle(context.get_output(0)); ctx_flat.first_transpose = trt.Permutation([1, 0, 2]); ctx_flat.reshape_dims = (max_pos, hidden)

        out_proj = graph_ops.add_bias_sum(network, graph_ops.add_matmul_rhs_constant(network, ctx_flat.get_output(0), hidden, hidden, weights[f"{pfx}.w_o"]), hidden, weights[f"{pfx}.b_o"])
    ...
    if verbose:
        print(f"[trtf-build] Building Marian encoder ({enc_layers}L, h={hidden}, heads={enc_heads})", file=sys.stderr)
...
def _add_marian_decoder_layer(*, network, hidden, cache_k, cache_v, cross_k, cross_v,
    attention_mask, encoder_mask, attn_scale_tensor, eps_tensor, weights, prefix,
    ...
    ckh = network.add_shuffle(ck_proj); ckh.reshape_dims = (max_enc_seq_len, num_heads, head_dim); ckh.second_transpose = trt.Permutation([1, 0, 2])
    cvh = network.add_shuffle(cv_proj); cvh.reshape_dims = (max_enc_seq_len, num_heads, head_dim); cvh.second_transpose = trt.Permutation([1, 0, 2])
    ...
    enc_mask_3d = network.add_shuffle(encoder_mask); enc_mask_3d.reshape_dims = (1, 1, max_enc_seq_len)
    cs_masked = network.add_elementwise(cs.get_output(0), enc_mask_3d.get_output(0), trt.ElementWiseOperation.SUM)
    csm = network.add_softmax(cs_masked.get_output(0)); csm.axes = 1 << 2
