...
class PersonaPlexPlugin:
    ...
    def build_extra_engines(
        ...
        if proj_parts:
            ...
            if verbose:
                print(f"[trtf-build] depth_projection: {all_proj.shape} "
        ...
        if emb_parts:
            ...
            if verbose:
                print(f"[trtf-build] audio_embeddings: {all_emb.shape} "
        ...
        if "text_emb" in weights:
            ...
            if verbose:
                print(f"[trtf-build] temporal_text_embedding: {text_emb.shape} "
        ...
        if "depformer_text_emb" in weights:
            ...
            if verbose:
                print(f"[trtf-build] depth_text_embedding: {depth_text_emb.shape} "
        ...
        if dep_emb_parts:
            ...
            if verbose:
                print(f"[trtf-build] depth_audio_embeddings: {all_dep_emb.shape} "
...
def _build_mimi_encoder_engine(
    ...
    *,
    ...
    if trt is None or graph_ops is None:
        if verbose:
            print("[trtf-build] Skipping Mimi encoder (TRT not available)",
                  file=sys.stderr)
        return None

    print("[trtf-build] Building Mimi encoder TRT engine ...", file=sys.stderr)
    ...
    if verbose:
        print(f"[trtf-build] Mimi encoder: {num_input_samples} samples -> "
              f"{ds_seq_len} frames x 32 codebooks", file=sys.stderr)

    plan = builder.build_serialized_network(network, config)
    if plan is None:
        print("[trtf-build] ERROR: Mimi encoder engine build failed",
              file=sys.stderr)
        return None

    plan_bytes = bytes(plan)
    print(f"[trtf-build] Mimi encoder engine built ({len(plan_bytes) / (1024*1024):.1f} MB)",
          file=sys.stderr)
    return plan_bytes
...
def _build_mimi_decoder_engine(
    ...
    *,
    ...
    if trt is None or graph_ops is None:
        if verbose:
            print("[trtf-build] Skipping Mimi decoder (TRT not available)",
                  file=sys.stderr)
        return None

    print("[trtf-build] Building Mimi decoder TRT engine ...", file=sys.stderr)
    ...
    if verbose:
        print(f"[trtf-build] Mimi decoder: {num_frames} frames x {num_codebooks} codebooks "
              f"(1 semantic + {num_acoustic} acoustic) -> "
              f"{output_samples} samples", file=sys.stderr)

    plan = builder.build_serialized_network(network, config)
    if plan is None:
        print("[trtf-build] ERROR: Mimi decoder engine build failed",
              file=sys.stderr)
        return None

    plan_bytes = bytes(plan)
    print(f"[trtf-build] Mimi decoder engine built ({len(plan_bytes) / (1024*1024):.1f} MB)",
          file=sys.stderr)
    return plan_bytes
