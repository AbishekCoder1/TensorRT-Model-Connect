...
def _build_streaming_encoder(weights: WeightDict, right_context: int, *,
    ...
    if verbose:
        ...
        print(f"[trtf-build] Building RNNT streaming encoder {step} "
...
class NemotronSpeechStreamingPlugin:
    ...
    def load_weights(self, model_dir: str, config: ModelConfig, *, precision: str = "fp32") -> WeightDict:
        ...
        mel_length = _cfg_int(config.raw.get("mel_length"), ncfg.get("trtf_mel_length"),
...
def _build_predictor(weights: WeightDict, *, verbose: bool = False) -> bytes:
    ...
    if verbose:
        print(f"[trtf-build] Building RNNT predictor ({pred_layers}L, h={pred_hidden})",
              file=sys.stderr)
    plan = builder.build_serialized_network(network, config)
    if plan is None:
...
def _build_joint(weights: WeightDict, *, verbose: bool = False) -> bytes:
    ...
    if verbose:
        print(f"[trtf-build] Building RNNT joint (enc={enc_hidden}, pred={pred_hidden}, "
...
def _build_mel_filterbank(weights: WeightDict, *, verbose: bool = False) -> bytes | None:
    ...
    try:
        ...
        try:
            ...
            print("[trtf-build] Warning: mel filterbank builder not available", file=sys.stderr)
            return None
    ...
    if verbose:
        print(f"[trtf-build] RNNT mel filterbank: {n_freq_bins}x{num_mel_bins}",
              file=sys.stderr)
